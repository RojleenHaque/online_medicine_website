  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (email === "admin@gmail.com" && password === "Admin@123") {
       ...
       response = await axios.post("http://localhost:4000/api/users/admin", {
          email,
          password,
        });
      } else {
    ...
    console.log("API response:", response);
      ...
      if (response.status === 200) {
        ...
        if (isAdmin) {
          navigate("/dashboard",'_blank'); // Admin dashboard route
        } else {
          navigate("/"); // Stay on home page for regular users
        }
      }
    } catch (error) {
